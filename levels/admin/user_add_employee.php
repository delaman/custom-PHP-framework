...
if (isset($_POST['submit'])) {
    ...
    */
    ...
    if($user->create()) {
        ...
        $log->priority = 1;
        $log->time = time();
        $log->log = "User employee added: {$employee->full_name()} added user {$user->username}.";
        $log->create();

        $session->message("User employee added successfully!");
        unset($_POST['submit']);
        redirect_to('user_list.php');
    } else {
        /* Failure */
        $session->message("User employee NOT added!");
    }
}
?>

<h2>Add Employee User</h2>
<form action="user_add_employee.php" enctype="multipart/form-data" method="POST">
    <fieldset class="fieldsetAdd">
        ...
        <ol>
            ...
            <li>
                ...
                <input id="employeeID" name="employeeID" class="text" type="text" size="5" />
            </li>
        </ol>
    </fieldset>
